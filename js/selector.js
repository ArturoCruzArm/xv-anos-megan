...
// GLOBAL VARIABLES
// ========================================
// Generate photo paths for 167 photos
const photos = Array.from({length: 167}, (_, i) => `images/foto_${String(i + 1).padStart(3, "0")}.webp`);

// LIMITS FOR MEGAN'S PACKAGE
const LIMITS = {
    impresion: 50,    // Máximo 50 fotos para impresión
    ampliacion: 0     // Ya está seleccionada - no sugerir más
    // redes_sociales: sin límite
};

const STORAGE_KEY = 'megan_xv_photo_selections';
let photoSelections = {};
...
function exportToJSON() {
    const exportData = {
        ...
        whatsapp: '4779203776',
        nombre: 'Megan',
        contratante: 'Cristina Padierna Sandoval',
        fecha_evento: '9 de agosto de 2025',
    ...
    a.href = url;
    a.download = `seleccion-megan-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
...
function generateTextSummary() {
    const stats = getStats();
    let summary = '📸 SELECCIÓN DE FOTOS - XV AÑOS MEGAN\n';
    summary += '═══════════════════════════════════════\n\n';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.option-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Skip invitacion - not available for Megan
...
// FEEDBACK MANAGEMENT
// ========================================
const FEEDBACK_KEY = 'megan_xv_feedback';
let feedbackData = {
    video: [],
    photos: []
};
...
function renderFeedbackLists() {
    ...
    if (feedbackData.photos.length === 0) {
        ...
        photoList.innerHTML = feedbackData.photos.map((item, index) => `
            <div style="display: flex; align-items: center; gap: 10px; padding: 10px; background: #f5f5f5; border-radius: 8px; margin-bottom: 8px;">
                <span style="font-weight: 600; color: #909090; min-width: 60px;">📸 #${item.photoNumber}</span>
                <span style="flex: 1; color: #333;">${item.change}</span>
